function ScenarioOutline(keyword, name, description, uri, line) {
  ...
  function buildScenario(example, index) {
    var newSteps = self.applyExampleRow(example, self.getSteps());
    var subScenario = Cucumber.Ast.Scenario(keyword, name + "." + index, description, uri, line);
  ...
  self.buildScenarios = function () {
    ...
    examplesCollection.syncForEach(function (examples) {
      ...
      exampleHashes.forEach(function (exampleHash, index) {
        scenarios.add(buildScenario(exampleHash, index));
      });
    });
